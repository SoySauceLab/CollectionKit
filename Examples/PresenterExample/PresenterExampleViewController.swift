...
class PresenterExampleViewController: CollectionViewController {
  override func viewDidLoad() {
    ...
    let imageProvider = CollectionProvider(
      ...
      layout: WaterfallLayout(columns:2).transposed().inset(by: bodyInset),
    ...
    let buttonsProvider = CollectionProvider(
      ...
      layout: FlowLayout(lineSpacing: 10).transposed().inset(by: UIEdgeInsets(top: 10, left: 16, bottom: 0, right: 16)),
